...
int main() {
    ...
    // Allocate memory for the array of pointers to pages arrays
    int** total_number_of_pages = (int**) malloc(total_number_of_shelves * sizeof(int*));
    
    // Initialize each shelf's pages array pointer to NULL
    ...
    while (total_number_of_queries--) {
        ...
        if (type_of_query == 1) {
            int x, y;
            scanf("%d %d", &x, &y);
            // Insert a new book with y pages at the end of shelf x
