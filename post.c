...
void send_all_acceptable_packages(town* source, int source_office_index, town* target, int target_office_index) {	
    ...
    for (int i = 0; i < src_office->packages_count; i++) {
        ...
        if (p->weight >= tgt_office->min_weight && p->weight <= tgt_office->max_weight) {
            // Package acceptable, transfer it
            // Reallocate target packages array
            tgt_office->packages = realloc(tgt_office->packages, sizeof(package) * (tgt_office->packages_count + 1));
            tgt_office->packages[tgt_office->packages_count] = *p; // copy package
            tgt_office->packages_count++;
        } else {
            // Keep package in source
            if (new_count != i) {
                src_office->packages[new_count] = *p;
            }
            new_count++;
        }
    }
    // Resize source's packages array
    src_office->packages = realloc(src_office->packages, sizeof(package) * new_count);
    src_office->packages_count = new_count;
}
